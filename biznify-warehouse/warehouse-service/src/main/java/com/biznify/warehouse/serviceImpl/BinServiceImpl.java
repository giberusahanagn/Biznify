...
import com.biznify.warehouse.dto.ProductDTO;
...
public class BinServiceImpl implements BinService {
    ...
    public List<BinLocationResponseDTO> allocateBins(BinAllocationRequestDTO request) {
        ...
        for (Bin bin : prioritizedBins) {
            ...
            // 🔁 NEW: Set max capacity dynamically if it is 0
            if (bin.getMaxUnitCapacity() == 0.0 && volumePerUnit > 0) {
                int estimatedCapacity = (int) Math.floor(bin.getVolumeCapacity() / volumePerUnit);
                bin.setMaxUnitCapacity((double) estimatedCapacity);
                log.warn("MaxUnitCapacity was zero. Dynamically estimated for bin {} as {}", bin.getBinCode(), estimatedCapacity);
            }
